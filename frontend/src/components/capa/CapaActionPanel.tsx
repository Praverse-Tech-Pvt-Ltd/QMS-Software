import { 
  ...
  Chip, Divider, TextField, MenuItem, Alert
} from "@mui/material";
...
const MOCK_ACTIONS = [
    { id: 1, action: "Replace Sensor Module", owner: "Maintenance", due: "2025-02-20", status: "Done" },
    { id: 2, action: "Update Calibration SOP", owner: "QA Lead", due: "2025-02-25", status: "In Progress" },
];

export default function CapaActionPanel({ readOnly = false }: CapaActionPanelProps) {
  return (
    <Paper sx={{ p: 3, borderRadius: 3, border: "1px solid rgba(0,0,0,0.06)" }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
        ...
        <Box>
            <Typography variant="h6" fontWeight={900}>Action Plan & Effectiveness</Typography>
      ...
      <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' } }}>
          <TextField 
            ...
            fullWidth multiline rows={2} disabled={readOnly}
            sx={{ gridColumn: '1 / -1' }}
          />
          <TextField 
            type="date" label="Check Due Date" defaultValue="2025-06-01" 
            fullWidth size="small" InputLabelProps={{shrink:true}} disabled={readOnly}
          />
          <TextField 
            select label="Outcome" defaultValue="Pending" 
          ...
          <TextField 
            label="Evidence Summary" placeholder="Results of the check..." 
